...
| V3 codebase scan — knk-plugin | knk-plugin (read-only); workspace: `docs/architecture/plugin-architecture.md`, `docs/reports/plugin-scan-2026-09-21.md`, `docs/guides/plugin-reading-guide.md` | 2026-09-21 | Per `v3-codebase-scan-instructions.md` §3. Key findings: gate structures fully implemented and most mature feature; siege minigame itself (capture/teams/scoring) not started, only inert placeholder fields; no GUI/menu code anywhere; storage migration to REST/MySQL confirmed complete; one dead command class, one stray committed `.class` file, a few stub/duplicate listeners flagged. Full detail in the scan report. |
