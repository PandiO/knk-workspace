...
| InventoryMenu Phase 2 — rendering engine core | plugin only: Bukkit-free runtime engine in `knk-core/.../core/menu/` (`RuntimeMenu`/`RuntimeMenuSection`/`RuntimeMenuItem`, `MenuTemplateAssembler`, `MenuSlotCalculator`, `MenuLayoutValidator`, `MenuSession`/`MenuSessionRegistry`, `MenuVariablePlaceholderText`) plus the Bukkit rendering/interaction layer in `knk-paper/.../paper/menu/` (`MenuRenderer`, `MenuItemBukkitMapper`, `MenuService`, `MenuClickListener`, `MenuLifecycleListener`, `OpenMenuContext(Registry)`), wired into `KnKPlugin` + a `/knk menu open\|page` dev-harness command (`knk-plugin` branch `claude/inventory-menu-phase-2-x4y98c`, forked from Phase 1's now-merged-in branch) | 2026-09-21 | 232 real JUnit tests (incl. a 200-iteration fuzz test on slot calculation, per DESIGN_REVIEW.md §1's explicit ask) pass via a standalone `javac` + JUnit-console harness against the actual production source files — `repo.papermc.io` is still blocked in this sandbox (confirmed 403 again via the proxy status endpoint, not routed around; `./gradlew :knk-core:compileJava` still fails the same way even offline, since `knk-core` already has a pre-existing `paper-api` dependency for gates code) so no module in this repo can go through a real Gradle build here. The new engine code was deliberately kept Bukkit-free (DESIGN_REVIEW.md's "isolate Bukkit types behind the Integration layer") specifically so this standalone-harness verification was possible at all. The Bukkit-facing `knk-paper` half (rendering/click handling) was hand-reviewed against this codebase's own existing, working Bukkit API call sites (`ItemBlueprintBukkitMapper`, `EnchantmentDefinitionBukkitMapper`, `PlayerListener`, `GateFireSystemTest`'s Bukkit-mocking pattern) rather than compiled — not run on a live dev server. **Needs before Phase 3:** a real `./gradlew build` + an actual `/knk menu open example.placeholder` dev-server smoke test once `paper-api` is reachable, since a rendering engine isn't fully "done" on hand-review alone. Design calls made without a docs answer, flagged for confirmation rather than assumed silently: (1) `MenuOverflowMode.SCROLL` and `WRAP` are both treated as "paginate" at the base-class level (only `HIDE` truncates without paging) - the requirements doc's WRAP/SCROLL distinction is left to the Phase 7 preset-section-type library; (2) `MenuListMode` (DEFAULT/LINEAR/GRID) is parsed and carried through but Phase 2 fills every section with one uniform row-major order regardless of mode, for the same reason; (3) a `MenuItemTemplate` with no `materialRefId` falls back to `Material.PAPER` with a logged warning rather than failing the render. |
