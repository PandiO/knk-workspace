...
**New finding during cleanup, unaudited by any earlier pass:** `origin/25/BoltSupplementation` (no `archive/` prefix) was a *separate ref* that happened to point at the exact same commit (`1831dc0`) as the just-deleted `archive/25/BoltSupplementation` — same content, not new unique work. It was also, unusually, set as this **repository's default branch on GitHub** (`origin/HEAD` pointed to it), which blocked its deletion on the first attempt ("refusing to delete the current branch").

**Resolved (2026-09-20, later same day):** the repo owner changed the GitHub default branch to `main`. `git push origin --delete "25/BoltSupplementation"` was retried and succeeded. Confirmed via `git fetch --prune` — the repo's branch list is now exactly `main`, `gate-animation-2`, `UserFeatures`, `integrate/archive/26/02/UserFeatures`, with no other local or remote refs left over. Audit and cleanup for `knk-web-app` are complete; only the two held-back branches remain open.
