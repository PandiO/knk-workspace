...
- **Area**: knk-web-api-v2 (backend), knk-web-app (frontend surfaces the error)
- **Reported**: 2026-09-10
...
- Controller: [FormSubmissionProgressController.cs:109](../Repository/knk-web-api-v2/Controllers/FormSubmissionProgressController.cs#L109) — `Delete(int id)`.
- Service: [FormSubmissionProgressService.cs:157](../Repository/knk-web-api-v2/Services/FormSubmissionProgressService.cs#L157) — `DeleteAsync(int id)`, no cascade/child handling, no try/catch around FK failures.
- Repository: [FormSubmissionProgressRepository.cs:66-74](../Repository/knk-web-api-v2/Repositories/FormSubmissionProgressRepository.cs#L66-L74) — `DeleteAsync(int id)` does a naive single-row `Remove` + `SaveChangesAsync`, with no awareness of child rows referencing it via `ParentProgressId`.
...
- **Area**: knk-web-app (frontend — root cause) / knk-web-api-v2 (backend — confirms the field is write-ignored anyway)
- **Reported**: 2026-09-10
...
- **Area**: knk-plugin-v2 (gate structure animation task)
- **Reported**: 2026-09-11
