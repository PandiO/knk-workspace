...
- **Status**: Implemented — folded into InventoryMenu Phase 7 per the developer's 2026-09-22 decision (see resolved question 6 below). See the InventoryMenu Phase 7 entry in `ACTIVE_SESSIONS.md`'s "Recently completed" table for full implementation detail and verification status.
...
- **Related docs**: `docs/specs/inventory-menu/{IMPLEMENTATION_PLAN.md,DESIGN_REVIEW.md §2.1,§2.5}`; see the InventoryMenu Phase 2, Phase 5, and Phase 7 entries in `ACTIVE_SESSIONS.md`'s "Recently completed" table for full implementation detail
...
- **Free-text input** (search, naming, custom values): an in-house
  anvil-style capture. Opens in place, captures text via the rename field,
  closes back into the originating menu with the result applied.
...
**Implementation approach (refined 2026-09-22): in-house, not third-party.**
The research pass initially proposed InvUI (chest-menu construction/
pagination) and AnvilGUI (md5lukas fork, free-text capture) as libraries to
add. The developer explicitly decided against taking on either as a real
dependency — build both patterns in-house instead, directly against the
Bukkit API: an InvUI-*inspired* clickable-item/pagination model built as an
extension of Phase 2's already-shipped in-house rendering engine (not a
library swapped in underneath it), and an AnvilGUI-*inspired* small,
self-contained anvil-capture component (open an anvil `Inventory`, listen
for a click on its output slot, read the renamed text, close and act) to
replace `ChatCaptureManager` for this specific use. No new third-party
jars, no shading/relocation concerns.

Full writeup: `docs/specs/inventory-menu/DESIGN_REVIEW.md` §2.5 (new) and
the updated §2.1. §2.1's own text-input question is now answered: an
in-house anvil-style capture, not Phase 5's `ChatCaptureManager` reuse —
that Phase 5 implementation is superseded, not just supplemented, by this
decision.

### Resolved questions (from the original research request)
1. ~~Right Bukkit-native in-GUI control for pagination/search/filter trigger?~~ → A plain clickable `MenuItem`, built on the existing in-house engine, wired through Phase 6's `ActionRegistry`. Confirmed nothing more exotic is needed.
2. ~~Chat capture or anvil GUI once the trigger is a real click?~~ → An anvil-style capture, built in-house (not the AnvilGUI library — see "Implementation approach" above). Phase 5's chat-capture approach is explicitly superseded, not kept as an option.
3. ~~Sequencing: fold into Phase 6/7, or pull forward as its own phase?~~ → **Decided (developer, 2026-09-22): keep Phase 6 to its original `IMPLEMENTATION_PLAN.md` scope** (generic `ActionRegistry`/`ConditionRegistry` wiring, click-time condition checks — no pagination/search/filter/confirm action types added as part of it). This interaction-model work is a **follow-up phase**, sequenced after Phase 6 lands `ActionRegistry` (since click-driven pagination/search/filter/confirm triggers all need it to do anything on click) but not folded into Phase 6 itself. Not yet decided: whether that follow-up is folded into Phase 7 ("Preset section/item library") or becomes its own phase between 6 and 7 — that's a real open question for whoever scopes it once Phase 6 is done, not decided here.
...
### Resolved questions (InventoryMenu Phase 7, 2026-09-22)
5. ~~In-house build vs. Phase 2's existing engine — how much is new work?~~ → Zero new rendering/pagination/layout engine work. Phase 2's `RuntimeMenuSection.resolveSlots`, `MenuSlotCalculator`, `MenuRenderer`, and `MenuItemBukkitMapper` were used entirely as-is — a pinned pagination/search/filter button is just a `MenuItemTemplate` with `SlotOverride` set and an `ActionBinding`, the same pinned-item mechanism Phase 5's persistent search/clear button already established. The only engine-level addition was plumbing a slot → owning-section map through `MenuRenderResult`/`OpenMenuContext` so a click handler can tell *which* section's pagination/search/filter state to act on (see the Phase 7 entry's "known gap" discussion) — not new layout/pagination logic, just routing.
6. ~~Does the follow-up become its own phase, or fold into Phase 7?~~ → Folded into Phase 7 (developer, 2026-09-22) — implemented together with the rest of Phase 7's preset-library work, not as a separate interim phase.

### Acceptance criteria
Met by InventoryMenu Phase 7: InventoryMenu pagination, search, and filters are all completable via clicks alone with the menu staying open (the in-house anvil-capture's in-place text entry for search/filter free-text input counts as "staying in the flow," not a command detour) — see `example.presets`' seed for a live demo of every control. `/knk menu ...` commands still work unchanged, calling the exact same `MenuService` methods the new click actions do — never a required path, per the decided UI-first/command-fallback principle. No new third-party InventoryMenu-related dependency was added to `knk-plugin`'s build files.

Confirmations are also click-driven end-to-end (`menu.confirm.request`/`.accept`/`.cancel` + `MenuSession.PendingConfirmation`), closing that part of DESIGN_REVIEW.md §2.5's interaction model too, though it wasn't explicitly named in this item's own symptom description (which only called out pagination/search/filter). Numeric input (+1/-1/shift-click quantity selection) remains explicitly unimplemented — blocked on no real quantity-bearing content existing anywhere in InventoryMenu yet, confirmed as a deliberate call (not an oversight) in the Phase 7 entry rather than assumed.
