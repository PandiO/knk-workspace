...
- **Status**: Ready — interaction model decided 2026-09-22 (research pass by another session), refined same day (developer: build the researched patterns in-house, not as third-party dependencies). Sequencing also decided same day: this is a follow-up phase after Phase 6, not folded into it. Not yet implemented — Phase 6 (`ActionRegistry`/`ConditionRegistry`) is a prerequisite. Two smaller scoping questions remain open, see below.
...
5. **In-house build vs. Phase 2's existing engine — how much is new work.** Now that this is confirmed in-house rather than a library swap, the real remaining question is scoping, not architecture-vs-library: how much of "InvUI-inspired" chest-menu/pagination behavior (e.g. scroll indicators, drag-to-reorder if wanted, any InvUI conventions worth matching) is genuinely new work on top of Phase 2's `MenuSlotCalculator`/`RuntimeMenuSection.resolveSlots`/`MenuRenderer`/`MenuItemBukkitMapper`, versus already covered by what's there. Whoever implements this should read those classes first rather than assume a rewrite is needed — Phase 2's engine was deliberately built to support exactly this kind of extension (pagination, slot calculation, and item mapping are already modular).
6. **New: does the follow-up become its own phase, or fold into Phase 7?** Raised by resolving question 3 above — not decided yet. Whoever picks up this item after Phase 6 lands should flag a recommendation to the developer rather than deciding unilaterally, the same way this item's original sequencing question was handled.

### Acceptance criteria
- Not yet fully defined pending open questions 3 and 5 above, but the interaction model itself is settled: once implemented, InventoryMenu pagination, search, filters, numeric input, and confirmations should all be completable via clicks alone with the menu staying open (the in-house anvil-style capture's in-place text entry for free-text/custom-amount input counting as "staying in the flow," not a command detour) — `/knk menu ...` commands should keep working afterward as an optional fallback, not be required for any of the above. No new third-party InventoryMenu-related dependency should be added to `knk-plugin`'s build files as part of implementing this.
