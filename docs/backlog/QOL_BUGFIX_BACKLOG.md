...
- **Status**: Ready — interaction model decided 2026-09-22 (research pass by another session), refined same day (developer: build the researched patterns in-house, not as third-party dependencies). Not yet implemented. One question from the original research request (sequencing, see open questions below) is still genuinely open.
...
3. **Sequencing**, unresolved: does this get folded into Phase 6 (click → action wiring) and Phase 7 (preset components) as originally planned, or pulled forward as its own phase? The research pass answered *what* the interaction model should be, not *when*/*how* to sequence adopting it relative to Phase 6/7's existing scope.
