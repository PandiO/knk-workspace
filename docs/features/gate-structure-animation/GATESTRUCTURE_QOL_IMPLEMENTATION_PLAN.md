...
**Status**: Plan drafted 2026-09-12. Item 4 complete (2026-09-12). Item 5 backend (5.1-5.3) complete (2026-09-12); plugin (5.4-5.7) and frontend (5.8-5.9) not started. Items 6-7 not started.
...
- **Known follow-ups once the plugin/frontend land**:
  - `WorldBoundFieldRenderer.tsx` (`knk-web-app`) still sends `gateStructureId` in the `GateBlockScan`/`GateOpenedBlockScan` task's `inputJson` - the backend's `WorldTaskService` now expects `gateDoorId` there instead.
  ...
  - 5.9's migration verification against entity 14 (and building its real second door) can only happen once the plugin/frontend can drive it.
